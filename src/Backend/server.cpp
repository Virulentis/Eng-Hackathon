...
std::string searchCanvas(std::string tail_url, std::string token){
    CURL* curl;
    CURLcode res;
...
void ModifyCourseIDsAndNamesFromCanvas(std::vector<std::string> *course_ids, std::vector<std::string> *course_names, std::string token){
    std::string readBuffer = searchCanvas("/courses", token);
...
std::string searchCanvasAssignments(std::string token){
    std::vector<std::string> course_ids;
    std::vector<std::string> course_names;
    ModifyCourseIDsAndNamesFromCanvas(&course_ids, &course_names, token);
    std::string output;
    for(int i=0;i<course_ids.size();i++){
        ...
        std::string readBuffer = searchCanvas(tail_url, token);
        // std::cout << readBuffer << std::endl; //DEBUG
...
int main() {
    ...
    ([](const crow::request& req){
        ...
        std::string result = parseOutputData(searchCanvasAssignments(received));
